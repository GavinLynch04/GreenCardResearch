...
# --------------------
def load_models():
    with open('Streamlit/pages/adaboost_model.pkl', 'rb') as f:
        ada = pickle.load(f)
    with open('Streamlit/pages/mapie.pkl', 'rb') as f:
        mapie = pickle.load(f)
    with open('Streamlit/pages/shap.pkl', 'rb') as f:
        shap_model = pickle.load(f)
    return ada, mapie, shap_model
...
def prediction_page():
    ...
    new_row = {
        'NAICS': st.session_state.get("NAICS"),
        'PW_LEVEL_9089': st.session_state.get("PW_LEVEL_9089"),
        'PW_AMOUNT_9089': st.session_state.get("PW_AMOUNT_9089"),
        'JOB_INFO_WORK_STATE': st.session_state.get("JOB_INFO_WORK_STATE"),
        'COUNTRY_OF_CITIZENSHIP': st.session_state.get("COUNTRY_OF_CITIZENSHIP"),
        'CLASS_OF_ADMISSION': st.session_state.get("CLASS_OF_ADMISSION"),
        'EMPLOYER_NUM_EMPLOYEES': st.session_state.get("EMPLOYER_NUM_EMPLOYEES"),
        'JOB_INFO_EDUCATION': st.session_state.get("JOB_INFO_EDUCATION"),
        'JOB_INFO_TRAINING': st.session_state.get("JOB_INFO_TRAINING"),
        'JOB_INFO_EXPERIENCE': st.session_state.get("JOB_INFO_EXPERIENCE"),
        'JOB_INFO_EXPERIENCE_NUM_MONTHS': st.session_state.get("JOB_INFO_EXPERIENCE_NUM_MONTHS"),
        'JOB_INFO_FOREIGN_ED': st.session_state.get("JOB_INFO_FOREIGN_ED"),
        'RI_LAYOFF_IN_PAST_SIX_MONTHS': st.session_state.get("RI_LAYOFF_IN_PAST_SIX_MONTHS"),
        'FOREIGN_WORKER_INFO_EDUCATION': st.session_state.get("FOREIGN_WORKER_INFO_EDUCATION"),
        'FW_INFO_YR_REL_EDU_COMPLETED': st.session_state.get("FW_INFO_YR_REL_EDU_COMPLETED"),
        'FW_INFO_REQ_EXPERIENCE': st.session_state.get("FW_INFO_REQ_EXPERIENCE")
    }
    ...
    st.session_state['input_features'] = input_df_encoded1.to_dict()
    ...
    st.markdown(f"""
    **Our model predicts:**  
    Your waiting time is estimated at **{prediction_val}** (with 90% confidence, between **{low_year}, {low_month}** and **{high_year}, {high_month}**).
    """)

    if st.button("Analyze Features"):
        st.session_state.page = "immigrationpage1dot5"
